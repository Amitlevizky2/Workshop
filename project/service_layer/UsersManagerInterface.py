...
class UsersManagerInterface:
    ...
    def register(self, username, new_username, password):
        logger.info("user %s called register with new_username:%s, password:%s", username, new_username, password)
    ...
    def login(self, username: str, login_username: str, password) -> bool:
        logger.info("user %s called login with login_username:%s, password:%s", username, login_username, password)
    ...
    def view_cart(self, username) -> Cart:
        logger.info("user %s called view_cart", username)
    ...
    def view_purchases(self, username):
        logger.info("user %s called view_purchases", username)
        return self.user_manager.view_purchases(username)

    def add_product(self, username, store_id, product, quantity):
        logger.info("user %s called add prdouct with store_id:%d, product_name:%s, quantity:%d", username, store_id, product.name, quantity)
    ...
    def remove_product(self, username, store_id, product, quantity):
        logger.info("user %s called remove product with store_id:%d, product_name:%s, quantity:%d", username, store_id, product.name, quantity)
    ...
    def view_purchases_admin(self, username, admin):
        logger.info("admin %s called view_purchases_admin for user %s", admin, username)
